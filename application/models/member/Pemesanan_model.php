...
class Pemesanan_model extends CI_Model
{
    ...
    public function insert($data)
    {
        ...
        $item = [
            ...
            'pick_up_delivery' => $data[0]['delivery']
        ];
        if (is_null($kode)) {
            $item['kd_pemesanan'] = 'BOOK-00001';
        } else {
            $arraykode = explode('-', $kode['kd_pemesanan']);
            $num = strlen(((int) $arraykode[1]) + 1);
            if ($num == 1) {
                $item['kd_pemesanan'] = 'BOOK-' . '0000' . (((int) $arraykode[1]) + 1);
            } else if ($num == 2) {
                $item['kd_pemesanan'] = 'BOOK-' . '000' . (((int) $arraykode[1]) + 1);
            } else if ($num == 3) {
                $item['kd_pemesanan'] = 'BOOK-' . '00' . (((int) $arraykode[1]) + 1);
            } else if ($num == 4) {
                $item['kd_pemesanan'] = 'BOOK-' . '0' . (((int) $arraykode[1]) + 1);
            } else if ($num == 5) {
                $item['kd_pemesanan'] = 'BOOK-' . (((int) $arraykode[1]) + 1);
            }
        }
